...
status, _, _ = Client().request('/criar-conta')
check(status == 404, 'Public registration removed')
status, html, _ = ana.submit('/admin/usuarios', '/admin/usuarios', {'nome': 'Teste ' + suffix, 'login': 'teste' + suffix, 'email': suffix + '@example.test', 'senha': 'TesteNovo!2026', 'confirmacao_senha': 'TesteNovo!2026', 'tipo': 'usuario', 'ativo': '1'})
check(status == 200 and 'Usuário cadastrado.' in html, 'Admin creates user')
new = Client()
status, html, _ = new.submit('/login', '/login', {'identifier': 'teste' + suffix, 'senha': 'TesteNovo!2026'})
check(status == 200 and 'Visão geral' in html, 'Created user can log in')
status, html, _ = new.request('/')
check(status == 200 and 'Visão geral' in html and 'Vamos organizar seu começo?' in html, 'Empty dashboard')
status, _, _ = new.request('/admin/usuarios')
check(status == 403, 'User management is admin only')
